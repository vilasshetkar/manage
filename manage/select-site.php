...
if(isset($_GET['domain'])){
  ...
  echo "<script>window.location = '/manage/index.php'</script>";
...
?>
        ...
        <div class="content">
<?php 
$result = mysql_query("SELECT * FROM `user_sites` WHERE user_id='$_SESSION[user_id]' and status='1'") or die(mysql_error());
?>
          <table border="0" cellpadding="0" cellspacing="0" class="border">
            <tr>
              <th scope="col">SELECT YOUR SITE TO EDIT</th>
            </tr>
  <?php		  	
	while($row = mysql_fetch_array($result)){ ?>
            <tr>
	...
	<?php echo '<a href="?domain='.$row['id'].'&amp;domain_name='.$row['domain'].'">';?> <?php echo $row['domain'];?> <?php echo '</a>';?>
