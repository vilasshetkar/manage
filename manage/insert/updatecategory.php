...
if (isset($_POST['insert'])=="editcategory") {
//  $id = $_GET['ID'];

$sql="UPDATE $_SESSION[user_id]_menu SET category_name='$_POST[category_name]', parent=$_POST[parent], category_desc='$_POST[category_desc]' WHERE id=".$_POST['id'];
if (!mysql_query($sql))
  {
...
<?php
  if (isset($_GET['UCAT'])){
?>
<form action="../../admin/insert/insert.php" method="post" name="form2" id="form2">
  <table align="center" class="border">
    ...
    <tr valign="baseline">
      ...
      <td>
      <?php 
$result= mysql_query("SELECT * FROM `$_SESSION[user_id]_category` WHERE id=".$_GET['UCAT']);
while ($row=mysql_fetch_assoc($result)){  
?>
<input name="category_name" type="text" id="category_name" value="<?php echo $row['category_name']; ?>" size="32" />
<?php } ?>
...
while ($row_cat = mysql_fetch_assoc($parent)){  
?>
        <option value="<?php echo $row_cat['id']?>" <?php if ($row_cat['id']==$_GET['PARENT']){ echo 'selected="selected"'; } ?>><?php echo $row_cat['category_name']?></option>
        <?php } ?>
      </select></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Category Description:</td>
      <td><textarea name="category_desc" cols="32" rows="2" id="category_desc"><?php 
$result= mysql_query("SELECT * FROM `$_SESSION[user_id]_category` WHERE id=".$_GET['UCAT']);
while ($row=mysql_fetch_assoc($result)){ echo $row['category_desc']; }?>
      </textarea></td>
    </tr>
  ...
  <input type="hidden" name="insert" value="editcategory" />
        <?php 
while ($row_id = mysql_fetch_assoc($result)){  
?>

  <input type="hidden" name="id" value="<?php echo $row_id['id']; ?>" />
<?php } ?>
