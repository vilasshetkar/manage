...
<!-- ************Edit Menu Function ************************************************************************************ -->
<?php 
$result = mysql_query("SELECT * FROM `$_SESSION[user_id]_content`WHERE domain=$_SESSION[domain]");
if(isset($_GET['SORTCAT'])){
	$result = mysql_query("SELECT * FROM `$_SESSION[user_id]_content` WHERE category=".$_GET['SORTCAT']);
	}
?>
<table width="100%" border="0" align="center" cellspacing="5">
  <tr>
      <td width="20%" align="right"><a href="newarticle.php" class="nav">New Article</a></td>
...
while($row = mysql_fetch_array($result)){ ?>
  <tr>
  <td width="5%" align="center" valign="middle" id="editmenu">
  <?php  echo $i++; ?>
  </td>
  <td width="5%" align="center" valign="middle">
  <a href="updatearticle.php?UAID=<?php echo $row['id'];?>&amp;CAT=<?php echo $row['category'];?>" class="nav" id="mname">Edit</a></td>
  <td width="7%" align="center" valign="middle"><a href="updatearticle.php?DEL=<?php echo $row['id'];?>" class="nav" id="delete">Delete</a></td>
  <td width="30%" align="left" valign="middle"><?php echo $row['title'];?></td>
  <td align="center" valign="middle">
  <?php if (!$row['category']==""){$cat = mysql_query("SELECT * FROM `$_SESSION[user_id]_category` WHERE id=$row[category]"); while($row_cat = mysql_fetch_array($cat)){ echo $row_cat['category_name'];}}?></td>
  <td align="center" valign="middle"><?php echo $row['author'];?></td>
  <td align="center" valign="middle"><?php echo $row['created'];?></td>
  <td align="center" valign="middle"><?php if ($row['status']==1){ echo 'Publish'; } else { echo 'Unpublish';}?></td>
  </tr>
  <?php }?>
</table>
