...
$result = mysql_query("SELECT `id`,`pwd`,`user_email`,`full_name`,`approved`,`user_level` FROM users WHERE 
           $user_cond
			AND `banned` = '0'
			") or die (mysql_error()); 
$num = mysql_num_rows($result);
  ...
  // Match row found with more than 1 results  - the user is authenticated. 
	...
	list($id,$pwd,$user_email,$full_name,$approved,$user_level) = mysql_fetch_row($result);
	...
	if(empty($err)){			
     ...
     // this sets session and logs user in  
	   ...
	   if(isset($_POST['remember'])){
		  ...
		  header("Location: ../select-site.php");
		 }
		}
		else
		{
